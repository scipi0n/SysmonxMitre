...
class event_17:
    ...
    def write_file(self):
        #EVENTO 17
        with open('myapp/sysmon_events_rules/Event17_18.xml', 'r') as f:
            event = f.read()
        ...
        for t in self.tecs:
            ...
            b_pn = Bs_event.find_all('PipeName', {'name': t})
            if b_pn: conjunto.append(b_pn)
        if conjunto:
            single_list = reduce(lambda x, y: x + y + ['\n'], conjunto)
            Bs_event.PipeEvent.extend(single_list)
            res = Bs_event.find_all('RuleGroup', {'name': '17_18_include'})
        ...
        if self.excludes == 1:
            rules_exclude = Bs_event.find_all('RuleGroup',{'name': '17_18_exclude'})
